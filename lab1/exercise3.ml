...
let rec typecheck (e : expr) : typ option =
  ...
  | If (e1, e2, e3) ->
    (match (typecheck e1, typecheck e2, typecheck e3) with
     | (Some(BoolT), t1, t2) when t1 = t2 -> t1
     | _ -> None)
;;
...
let rec trystep (e : expr) : result =
  ...
  | If (e1, e2, e3) ->
      (match trystep e1 with
       ...
       | Val -> let Bool judge = e1 in 
                Step(if judge then e2 else e3))
...
assert (eval (If(Bool true, Int 1, Int 2)) = (Int 1));
assert (eval (If(Iszero(Int 0), Binop(Add, Int 2, Int 3), Int 2)) = (Int 5));
